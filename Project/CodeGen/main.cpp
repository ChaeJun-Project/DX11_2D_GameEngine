...
int main()
{
	...
	wstring strCppPath = strProjPath + L"Script_Lib\\ScriptManager.cpp";
	wstring strHeaderPath = strProjPath + L"Script_Lib\\ScriptManager.h";

	// 1. 현재 존재하는 모든 스크립트를 알아내야함.
	wstring strScriptIncludePath = CPathMgr::GetIncludePath();
	wstring strScriptCode = strScriptIncludePath + L"Script_Lib\\";
	...
	_wfopen_s(&pExeptList, L"Exclude_List.txt", L"r");
	...
	_wfopen_s(&pScriptListFile, L"Script_List.txt", L"r");
	...
	fwprintf_s(pFile, L"class Script;\n\n");

	fwprintf_s(pFile, L"class ScriptManager\n{\n");
	fwprintf_s(pFile, L"public:\n\tstatic void GetScriptInfo(vector<wstring>& _vec);\n");
	fwprintf_s(pFile, L"\tstatic Script * GetScript(const wstring& _strScriptName);\n");
	fwprintf_s(pFile, L"\tstatic Script * GetScript(UINT _iScriptType);\n");
	fwprintf_s(pFile, L"\tstatic const wchar_t * GetScriptName(Script * _pScript);\n};\n");
	...
	// 두번째 함수 작성
	fwprintf_s(pFile, L"Script * ScriptManager::GetScript(const wstring& _strScriptName)\n{\n");
	...
	// 세번째 함수
	fwprintf_s(pFile, L"Script * ScriptManager::GetScript(UINT _iScriptType)\n{\n");
	...
	// 네번째 함수
	fwprintf_s(pFile, L"const wchar_t * ScriptManager::GetScriptName(Script * _pScript)\n{\n");
	...
	_wfopen_s(&pFile, L"Script_List.txt", L"w");
