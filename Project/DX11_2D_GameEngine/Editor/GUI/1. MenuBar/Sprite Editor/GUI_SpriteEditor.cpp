	...
	if (ImGui::Begin("Sprite Editor", &m_is_active))
	{
		...
		if (ImGui::Button("Save Sprite Animation", ImVec2(170.0f, 0.0f)))
		{
			if (m_p_animation2D != nullptr)
				FileFunction::SaveFile(ANIMATION_PATH, m_p_animation2D->GetResourceName(), FileType::Animation);
		}
		ImGui::SameLine();
		...
		if (ImGui::Button("Load Sprite Animation", ImVec2(170.0f, 0.0f)))
		{
			...
			if (!animation2D_path.empty())
				m_p_animation2D = resource_manager->LoadFromFile<Animation2D>(animation2D_path);
		}
		...
		if (m_p_atlas_texture != nullptr)
		{
			...
			if (ImGui::Button("Preview Sprite Animation", ImVec2(170.0f, 0.0f)))
			{
				if (m_p_animation2D != nullptr && m_p_animation2D->HasAnimationFrame())
				{
					m_p_gui_sprite_player->m_is_active = true;
					m_p_gui_sprite_player->m_p_current_animation2D = m_p_animation2D;
					m_p_gui_sprite_player->m_frame_index = 0;
					m_p_gui_sprite_player->curret_animation_state = static_cast<UINT>(AnimationState::Stop);
				}
			}
...
void GUI_SpriteEditor::ShowAnimationEditor()
{
	...
	ImGui::Text("Sprite Animation Name");
	ImGui::SameLine();
	ImGui::PushItemWidth(150.0f);
	...
	if (ImGui::Button("New Sprite Animation", ImVec2(150.0f, 0.0f)))
	{
		if (animation_name.size())
		{
			m_p_animation2D = ResourceManager::GetInstance()->CreateAnimation2D(animation_name); //Game Object의 이름을 수정한 경우에만 수행
			m_p_animation2D->SetAtlasTexture(m_p_atlas_texture);
			animation_name = "";
		}
	}
	...
	if (m_p_animation2D != nullptr)
	{
		...
		//	std::string selected_animation_name = m_p_animation2D->GetResourceName();
		...
		if (m_p_animation2D->HasAnimationFrame())
		{
			...
			//Duration
			ShowFloat(m_p_animation2D->GetResourceName(), "Duration", animation2D_frame.duration, 150.0f);
